...
public class SmelterTile extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && hasStack() && worldObj.getWorldTime() % 100 == 0){
            ...
            if (stack.getItem() == Item.getItemFromBlock(Blocks.coal_ore)){
                ...
                stack = null; // Удаляем стэк
                markDirty(); // Сохраняем
            } else if (stack.getItem() == Item.getItemFromBlock(Blocks.iron_ore)) { // Если предмет является блоком железной руды, то создаём сущность предмета, которая содержит в себе стек с предметом "железный слиток"
    ...
    public void handleInputStack(EntityPlayer player, ItemStack stack){
        ...
        if(hasStack()){
            ...
            this.stack = null;
        } else if (stack != null) { // Если стек в руке не равен null, то он будет положен в кол-ве 1 шт., в StorageTile
